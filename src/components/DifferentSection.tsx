...
function SpotlightCard({
  ...
  return (
    <div
      ...
      className={`relative flex-1 flex flex-col gap-4 px-8 py-6 bg-[#f5f4ee] overflow-hidden ${point.borderClass}`}
      style={{
        outline: "1px dashed",
        outlineOffset: "-1px",
...
export default function DifferentSection() {
  return (
    <section className="bg-[#f0eee6] px-16 shrink-0">
      <div className="border-l border-r border-[rgba(36,36,36,0.1)] px-8 py-[58px] flex flex-col gap-10 items-center">
        {/* Header */}
        <div className="flex flex-col gap-2.5 items-center">
          ...
          <h2 className="font-serif text-[48px] text-[#242424] text-center leading-[1.2] tracking-[-1.44px] max-w-[800px] mt-1">
        ...
        <div className="flex items-stretch w-full">
          {points.map((point) => (
            <SpotlightCard key={point.index} point={point} />
          ))}
        </div>
      </div>
    </section>
